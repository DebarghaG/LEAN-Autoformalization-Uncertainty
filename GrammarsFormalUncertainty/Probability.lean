...
/-!
# Probability Computations for PCFGs

This module provides probability computations and entropy calculations:
- Maximum Likelihood Estimation (MLE) for rule probabilities
- Laplace (additive) smoothing
- Shannon entropy computations
- Cross-entropy and perplexity metrics
-/
...
-/
def computeProbabilities (counts : PCFGCounts) (startSymbols : Array NonTerminal := #[]) : PCFG := Id.run do
...
-/
def computeProbabilitiesSmoothed (counts : PCFGCounts) (alpha : Float := 1.0)
    (startSymbols : Array NonTerminal := #[]) : PCFG := Id.run do
  let mut rules : Std.HashMap NonTerminal (Array WeightedRule) := {}
  let mut nonTerminals : Std.HashSet NonTerminal := {}
  let mut terminals : Std.HashSet Terminal := {}
