...
/-!
# Basic Data Structures for PCFGs

This module defines the core data structures for Probabilistic Context-Free Grammars:
- `Terminal`: Abstract terminal symbols (keywords, identifiers, literals)
- `NonTerminal`: Non-terminal symbols (production rule left-hand sides)
- `Symbol`: Either a terminal or non-terminal
- `ProductionRule`: A production rule with occurrence count
- `WeightedRule`: A production rule with computed probability
- `PCFGCounts`: Accumulated counts during training
- `PCFG`: The final probabilistic grammar
-/

namespace GrammarsFormalUncertainty
...
/-- NonTerminal is a string representing a syntax node kind -/
