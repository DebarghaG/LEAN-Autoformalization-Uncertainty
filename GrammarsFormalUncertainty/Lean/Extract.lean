...
/-!
# PCFG Extraction from Lean Syntax Trees

This module provides functions to extract PCFG production rules from Lean syntax trees.
It bridges between Lean's internal `Syntax` type and the library's generic types.
-/
...
/-- Convert a Lean.Name to our NonTerminal (String) -/
def nameToNT (n : Name) : NonTerminal := n.toString
...
def syntaxToSymbol (stx : Syntax) : Option Symbol :=
  if isTerminalSyntax stx then
    ...
    match stx with
    | .node _ k _ =>
      if k != nullKind then some (.nonTerminal (nameToNT k)) else none
    | _ => none
...
/-- Default start symbols for Lean grammars -/
def defaultLeanStartSymbols : Array NonTerminal :=
  #["Lean.Parser.Module.module",
    "Lean.Parser.Command.declaration",
    "term",
    "tactic"]

/-- Compute probabilities with default Lean start symbols -/
def computeLeanProbabilities (counts : PCFGCounts) : PCFG :=
  computeProbabilities counts defaultLeanStartSymbols

/-- Compute smoothed probabilities with default Lean start symbols -/
def computeLeanProbabilitiesSmoothed (counts : PCFGCounts) (alpha : Float := 1.0) : PCFG :=
  computeProbabilitiesSmoothed counts alpha defaultLeanStartSymbols

end GrammarsFormalUncertainty.Lean
