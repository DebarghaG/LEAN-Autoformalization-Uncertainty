...
Command-line interface for extracting PCFGs from Lean source files and
...
structure Options where
  ...
  inspectNt : Option NonTerminal := none
...
def printUsage : IO Unit := do
  IO.println "Usage: gfu <input-path> [options]"
  IO.println ""
  IO.println "Extract a PCFG from Lean source files (GrammarsFormalUncertainty)."
  IO.println ""
...
def parseArgs (args : List String) : ParseResult :=
  let rec go (args : List String) (opts : Options) : ParseResult :=
    ...
    | "--inspect" :: name :: rest =>
      go rest { opts with inspectNt := some name }
    | "--entropy" :: rest =>
  ...
  | .options opts =>
    try
      ...
      let pcfg := if opts.smoothingAlpha > 0 then
        computeLeanProbabilitiesSmoothed counts opts.smoothingAlpha
      else
        computeLeanProbabilities counts
...
/-- Top-level main entry point for the executable -/
def main (args : List String) : IO UInt32 :=
  GrammarsFormalUncertainty.Lean.main args
